...
| trust=let_form id=oident arg_l=list(paren(ann_binder)) rel=bsymbol r_ty=ty EQ iexp=exp IN oexp=exp
   { fun env ->
     ...
     mk_loc (EC.EcLocation.make $startpos $endpos) @@
...
| LET REC fid=oident arg_l=nonempty_list(paren(ann_binder)) tc=term_ann rel=bsymbol r_ty=ty  EQ iexp=exp IN oexp=exp
   { fun env ->
     ...
     let fix_exp = mk_loc (EC.EcLocation.make $startpos $endpos) @@
...
| PIPE pat=pattern ARROW pe=exp
   {
     fun env ->
       ...
       let lpat         = mk_loc (EC.EcLocation.make $startpos $endpos) (cs, bil) in
       (lpat, pe p_env)
   }
...
| cid=constructor
                { fun env ->
                      let cs = cid env in
                      mk_loc (EC.EcLocation.make $startpos $endpos) (ECs cs)
                }
...
| args=paren(plist2(ty, STAR))
    {
      fun env ->
        ...
        mk_loc (EC.EcLocation.make $startpos $endpos) @@ (C.ty_tuple args, None)
    }
...
| x=X {
    { pl_desc = x;
      pl_loc  = EC.EcLocation.make $startpos $endpos;
    }
  }
;
