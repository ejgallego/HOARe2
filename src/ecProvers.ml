...
let get_prover name : (string * Whyconf.config_prover * Driver.driver) =
...
let execute_task (pi : prover_infos) task =
  ...
  let run i prover =
    try
      (* Format.printf "[info] Warning: Starting prover %s\n%!" prover; *)
        ...
        for i = 0 to (Array.length pcs) - 1 do
          ...
          | Some (_prover, pc) ->
              if CP.prover_call_pid pc = pid then begin
                ...
                match ans with
                | CP.Valid   -> status := Some (true, _prover)
                | CP.Invalid -> status := Some (false, _prover)
    ...
    (fun () ->
      for i = 0 to (Array.length pcs) - 1 do
        ...
        | Some (_prover,pc) ->
            ...
            pcs.(i) <- None;
            (* TODO: We need to use the wrapper here. *)
