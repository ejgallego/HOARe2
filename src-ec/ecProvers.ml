...
module Version : sig
  ...
  val _compare : version -> version -> int
...
*)
  ...
  let _compare (v1 : version) (v2 : version) =
...
let test_if_evict_prover _tests _prover = None
(*
...
module Config : sig
  ...
  let get prover =
    load ();
    let config = oget !theconfig in
    let { Whyconf.driver ; _ } = Whyconf.get_prover_config config prover in
    Whyconf.load_driver (oget !themain) (oget !thew3_env) driver []
...
type notify = loglevel -> string Lazy.t -> unit
...
let get_driver = Config.get

(* -------------------------------------------------------------------- *)
let array_count f a =
  let c = ref 0 in
  for i = 0 to Array.length a - 1 do
    if f a.(i) then incr c
  done;
  !c
...
let execute_task ?(notify : notify option) (pi : prover_infos) task =
  ...
  EcUtils.try_finally
    (fun () ->
      ...
      let status = ref 0 in
      let pr_names = ref [] in
      let alives = ref (array_count is_some pcs) in
      ...
      while (   (!alives <> 0 || not (Queue.is_empty pqueue))
        ...
        for i = 0 to (Array.length pcs) - 1 do
          ...
          | Some (prover, pc) ->
           ...
           if !status < 0 then Some (false, !pr_names)
      else if !status = 0 then None
      else if !status < pi.pr_quorum then None
      else Some (true, !pr_names))
