...
Parameter c1: ((list ty)* (list ty)* nat)%type.

Parameter c2: ((list ty)* (list ty)* nat)%type.
...
Axiom i_rtype : (i1 = i2) /\ ((0%Z <= (nat_to_int i1))%Z /\
  ((nat_to_int i1) < (nat_to_int m))%Z).

Axiom ref__rtype : (c1 = (rs1, ss1, i1)) /\ (c2 = (rs2, ss2, i2)).

(* Why3 goal *)
Theorem ty_goal : (c1 = c2) /\
  (((list.Length.length (fst3 c1)) = (nat_to_int predm)) /\
  (((list.Length.length (fst3 c2)) = (nat_to_int predm)) /\
  (((list.Length.length (snd3 c1)) = (nat_to_int m)) /\
  (((list.Length.length (snd3 c2)) = (nat_to_int m)) /\
  (((nat_to_int (thd3 c1)) < (nat_to_int m))%Z /\
  ((nat_to_int (thd3 c2)) < (nat_to_int m))%Z))))).
Proof.
  pose proof ref__rtype as [p1 p2].
  pose proof i_rtype    as [pi1 pi2].
  rewrite p1, p2; simpl; repeat split; auto.
  admit.
