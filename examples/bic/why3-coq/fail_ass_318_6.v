...
(* Why3 assumption *)
Definition distPres {a:Type} {a_WT:WhyType a} (mu:(distr a)) (f:(a -> (distr
  a))): Prop := (((sampleAndApply mu) f) = mu).
...
Fixpoint allPres {a:Type} {a_WT:WhyType a} (mu:(distr a)) (l:(list (a ->
  ...
  match l with
  | Init.Datatypes.nil => True
  | (Init.Datatypes.cons x xs) => (distPres mu x) /\ (allPres mu xs)
  end.
...
Axiom mapIdxMoves_rtype : forall (sz:Z), forall (f:(nat -> (ty -> (distr
  ty)))), (forall (idx1:nat), ((nat_to_int idx1) < sz)%Z -> (distPres mu (f
  idx1))) -> ((0%Z <= sz)%Z -> ((list.Length.length ((mapIdxMoves sz)
  f)) = sz)).
...
Axiom mapWithIdxValue_rtype : forall (xs1:(list ty)) (xs2:(list ty)),
  ...
  ((mapWithIdxValue xs1) f)) = ((f idx1) (list.NthNoOpt.nth idx1 xs1)))) /\
  ((forall (idx1:Z), ((0%Z <= idx1)%Z /\
  (idx1 < (list.Length.length xs1))%Z) -> ((list.NthNoOpt.nth idx1
  ((mapWithIdxValue xs2) f)) = ((f idx1) (list.NthNoOpt.nth idx1 xs2)))) /\
  forall (idx1:Z), ((0%Z <= idx1)%Z /\
  (idx1 < (list.Length.length xs1))%Z) -> ((differAt (int_to_nat idx1) xs1
  xs2) -> ((allBut (int_to_nat idx1) ((mapWithIdxValue xs1)
  f)) = (allBut (int_to_nat idx1) ((mapWithIdxValue xs2) f))))))).
...
Axiom vcg_rtype : forall (reps1:(list ty)) (reps2:(list ty)),
  ...
  ((list.Length.length surs11) = (nat_to_int m))) -> forall (wts:(ty ->
  (ty -> R))), (forall (i:Z), ((0%Z <= i)%Z /\
  ((i <= (nat_to_int predm))%Z /\ (differAt (int_to_nat i) reps1 reps2))) ->
  ((((wts (list.NthNoOpt.nth i reps1)) (list.NthNoOpt.nth i (fst (((vcg
  reps2) surs21) wts)))) - (list.NthNoOpt.nth i (snd (((vcg reps2) surs21)
  wts))))%R <= (((wts (list.NthNoOpt.nth i reps1)) (list.NthNoOpt.nth i
  (fst (((vcg reps1) surs11) wts)))) - (list.NthNoOpt.nth i (snd (((vcg
  reps1) surs11) wts))))%R)%R) /\ ((list.Permut.permut surs11 (fst (((vcg
  reps1) surs11) wts))) /\ (list.Permut.permut surs21 (fst (((vcg reps2)
  surs21) wts)))).
...
Axiom vcgout_rtype : (forall (i:Z), ((0%Z <= i)%Z /\
  ...
  vcgin2))) -> (((expect2 (mbind ((repeatM predn) mu)
  (fun (others:(list ty)) => (munit ((value (list.NthNoOpt.nth i vcgin1))
  (alg (insertAt idx (list.NthNoOpt.nth i (fst vcgout2))
  others))))))) - (list.NthNoOpt.nth i (snd vcgout2)))%R <= ((expect2
  (mbind ((repeatM predn) mu) (fun (others:(list ty)) => (munit ((value
  (list.NthNoOpt.nth i vcgin1)) (alg (insertAt idx (list.NthNoOpt.nth i
  (fst vcgout1)) others))))))) - (list.NthNoOpt.nth i
...
(* Why3 goal *)
Theorem ty_goal : (report1 = truety1) -> (((((fun (t:ty) => fun (s:ty) =>
  (expect2 (mbind ((repeatM predn) mu) (fun (others:(list ty)) =>
  (munit ((value t) (alg (insertAt idx s others)))))))) truety1)
  (fst res2)) - (snd res2))%R <= ((((fun (t:ty) => fun (s:ty) => (expect2
  (mbind ((repeatM predn) mu) (fun (others:(list ty)) => (munit ((value t)
  (alg (insertAt idx s others)))))))) truety1)
