...
Axiom rsmdet_rtype : forall (idx:nat), forall (coins1:((list ty)* (list ty)*
  ...
  report21)))%R <= ((((fun (t:ty) => fun (s:ty) => (expect2 (mbind ((repeatM
  predn) mu) (fun (others1:(list ty)) => (munit ((value t) (alg (insertAt idx
  s others1)))))))) truety11) (fst ((((rsmdet idx) coins1) truety11)
  report11))) - (snd ((((rsmdet idx) coins1) truety11) report11)))%R)%R).

Axiom others_rtype : forall (idx:nat),
  ((nat_to_int idx) <= (nat_to_int predn))%Z -> (distPres mu (others idx)).
